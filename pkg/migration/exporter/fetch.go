...
func (e *UnstructuredFetcher) shouldSkip(r unstructured.Unstructured) bool { // nolint:gocyclo // Relatively simple logic.
	...
	if r.GetKind() == "Secret" {
		if r.GetName() == "crossplane-root-ca" || r.GetName() == "crossplane-tls-client" || r.GetName() == "crossplane-tls-server" {
			// Skip Crossplane core TLS secrets. They are specific to the
			// Crossplane installation and should not be exported.
			// Provider/function TLS secrets will be skipped based on the owner
			// reference below and generated by the package manager on the
			// target cluster.
			return true
		}
