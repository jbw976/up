...
const testConfigurationName = "configuration-getting-started"
	...
	tcs := map[string]struct {
		...
		"Push": {
			...
			installFn: func(_ context.Context, _ client.Client, name string, tag name.Tag, _ async.EventChannel) error {
				if name != testConfigurationName {
					return errors.Errorf("wrong configuration name: expected %q got %q", testConfigurationName, name)
				}
		...
		"SideloadWithTag": {
			...
			devCtp: &ctp.MockSideloadingDevControlPlane{
				...
				SideloadFn: func(_ context.Context, _ project.ImageTagMap, tag name.Tag) error {
					wantTag := "v1.2.3"
					if tag.TagStr() != wantTag {
						return errors.Errorf("wrong tag in sideload: expected %q got %q", wantTag, tag.TagStr())
					}
					wantRepo, gotRepo := expectedTag.Context().Name(), tag.Context().Name()
					if wantRepo != gotRepo {
						return errors.Errorf("wrong repository in sideload: expected %q got %q", wantRepo, gotRepo)
			...
			installFn: func(_ context.Context, _ client.Client, name string, tag name.Tag, _ async.EventChannel) error {
				...
				wantTag := "v1.2.3"
				if tag.TagStr() != wantTag {
					return errors.Errorf("wrong tag in install: expected %q got %q", wantTag, tag.TagStr())
				}
				return nil
			},
		},
