...
// v2ChartValues reflects image tags from the UXP v2 Helm chart (crossplane) values.yaml.
type v2ChartValues struct {
	Image struct {
		Tag string `json:"tag"`
	} `json:"image"`
	Upbound struct {
...
// GetV2RuntimeTags pulls the UXP v2 Helm chart and returns crossplane and controller-manager
...
func GetV2RuntimeTags(chartRef, version, username, password string) (crossplaneTag, controllerManagerTag string, err error) {
	return getV2RuntimeTagsWithDeps(chartRef, version, username, password, loader.Load, oci.DefaultPullFunc)
}

func getV2RuntimeTagsWithDeps(chartRef, version, username, password string, loaderFunc oci.LoaderFunc, pullFunc oci.PullFunc) (string, string, error) {
	loadedChart, err := oci.LoadChart(chartRef, version, loaderFunc, pullFunc, username, password)
	if err != nil {
		return "", "", err
	}
	return parseV2RuntimeTags(chartRef, version, loadedChart)
}

func parseV2RuntimeTags(chartRef, version string, loadedChart *chart.Chart) (string, string, error) {
	appVersion := ""
	...
	cx := resolveImageTag(v.Image.Tag, appVersion)
	cm := resolveControllerManagerTag(v.Upbound.Manager.Image.Tag, cx, appVersion)
	if cx == "" {
		return "", "", fmt.Errorf("failed to resolve crossplane image tag for chart %s:%s (empty image.tag and appVersion)", chartRef, version)
	}
	if cm == "" {
		return "", "", fmt.Errorf("failed to resolve controller-manager image tag for chart %s:%s (empty upbound.manager.image.tag and appVersion)", chartRef, version)
	}
	return cx, cm, nil
}

// resolveControllerManagerTag mirrors Helm defaults: explicit upbound.manager.image.tag, else when
...
func resolveControllerManagerTag(managerValuesTag, crossplaneTag, appVersion string) string {
...
func resolveImageTag(valuesTag, appVersion string) string {
