...
//go:embed testdata/upboundV1alpha1.yaml
var projectFileV1alpha1 []byte

//go:embed testdata/upboundV2alpha1.yaml
var projectFileV2alpha1 []byte

func TestCompositionValidationPipelineWithProjectV1alpha1(t *testing.T) {
		...
		"ValidPipeline": {
			...
			args: args{
				workspace: func() *workspace.Workspace {
					f := afero.NewMemMapFs()
					_ = afero.WriteFile(f, "/upboundV1alpha1.yaml", projectFileV1alpha1, 0o644)
...
func TestCompositionValidationPipelineWithProjectV2alpha1(t *testing.T) {
	objScheme, _ := scheme.BuildObjectScheme()
	metaScheme, _ := scheme.BuildMetaScheme()
	ctx := t.Context()
	...
	cases := map[string]struct {
		...
		"FunctionRefMissingDependency": {
			...
			args: args{
				workspace: func() *workspace.Workspace {
					f := afero.NewMemMapFs()
					_ = afero.WriteFile(f, "/upboundV2alpha1.yaml", projectFileV2alpha1, 0o644)
					ws, _ := workspace.New("/", workspace.WithFS(f))
					return ws
				}(),
				data: &v1.Composition{
